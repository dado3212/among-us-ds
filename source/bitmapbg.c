...
void NF_Load16bImgData(const char* file, u8 slot, u16 x, u16 y, u8 type) {
	...
	// Variable para el tama�o de archivo
	u32 size = 0;
	...
	if (file_id) {	// Si el archivo existe...
		...
		size = ftell(file_id);
		rewind(file_id);
		// Si excede del tama�o maximo (128kb), error
	...
	NF_BG16B[slot].size = size;		// Guarda el tama�o
...
void NF_Unload16bitsBg(u8 slot) {
	...
	NF_BG16B[slot].size = 0;		// Tama�o a 0
...
void NF_Load8bitsBg(const char* file, u8 slot) {
	...
	if (file_id) {	// Si el archivo existe...
		...
		size = ftell(file_id);
		rewind(file_id);
		// Si excede del tama�o maximo (64kb), error
	...
	fclose(file_id);		// Cierra el archivo
	NF_BG8B[slot].data_size = size;		// Guarda el tama�o del buffer
	...
	if (file_id) {	// Si el archivo existe...
		...
		size = ftell(file_id);
		rewind(file_id);
		// Si la paleta tiene un tama�o inferior a 512, ajusta el tama�o
	...
	fclose(file_id);		// Cierra el archivo
	NF_BG8B[slot].pal_size = size;		// Guarda el tama�o del buffer
