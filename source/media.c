...
void NF_LoadBMP(const char* file, u8 slot) {
	...
	typedef struct {
		u32 bmp_size;		// Tama�o en bytes del BMP
		u16 res_a;			// Reservado
		u16 res_b;			// Reservado
		u32 offset;			// Offset donde empiezan los datos
		u32 header_size;	// Tama�o de la cabecera (40 bytes)
		...
		u32 raw_size;		// Tama�o de los datos en RAW despues de la cabecera
	...
	u32 size = 0;		// Tama�o del buffer de 16 bits (en bytes)

	// Habilita el buffer de destino (u16 de alto x ancho del tama�o de imagen)
	...
	switch (bmp_header.bpp) {
		...
		case 8:		// 8 bits por pixel
			// Calcula el tama�o de la paleta
